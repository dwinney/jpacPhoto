...
namespace jpacPhoto
{
    namespace fixed_spin
    {
        class pion_exchange : public raw_inclusive_process
        {
            ...
            inline double minimum_M2(){ return M_PROTON + M_PION; };
            ...
            {
                double sigmatot    = _sigma->evaluate(M2, M2_PION) * 1E6; // in nb
            ...
            {
                return exp((t - TMINfromM2(M2_PROTON))/_LamPi/_LamPi) * (_g/sqrt(_mX2))*(_mX2 - t)/2;
            };
            ...
            double _LamPi = 0; // Exponential cut-off
        };
    };
};
