...
namespace jpacPhoto
{
    ...
    class lorentz_tensor
    {
        ...
        : _conj(if_conj), _indices(list), _N(1)
        {};
        ...
        inline complex element(std::array<lorentz_index,Rank> vals)
        {
            ...
            // Apply the overall normalization
            result *= _N;

            // Complex conjugate if requested
    ...
    inline complex flatten(lorentz_tensor<0> T)
    {
        return T._N;
    };

    inline lorentz_vector flatten(lorentz_tensor<1> T)
    {
