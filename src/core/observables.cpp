...
void jpacPhoto::amplitude::update_cache(double s, double t)
{
    ...
    if (  (abs(_cached_s - s) < _cache_tolerance) && 
          ...
          (abs(_cached_mR - _kinematics->get_recoil_mass()) < _cache_tolerance) // also check the baryon mass
       )
    {
        return; // do nothing
    }
    else // save a new set
    {
        ...
        for (int i = 0; i < n; i++)
        {
            ...
            if (i < n/2) amp = helicity_amplitude(_kinematics->helicities(i), s, t);
            else         amp = parity_phase(i) * _cached_helicity_amplitude[n - 1 - i];
            _cached_helicity_amplitude.push_back(amp);
        };
