...
namespace jpacPhoto
{
    namespace covariant
    {
        class pseudoscalar_exchange : public one_meson::raw_amplitude
        {
            public:

            // Constructor we specify the exchange particle mass
            pseudoscalar_exchange(one_meson::amplitude_key key, one_meson::kinematics xkinem, double exchange_mass, std::string id = "pseudoscalar_exchange")
            : one_meson::raw_amplitude(key, xkinem, id), _mEx(exchange_mass)
            {
                initialize(3);
            }
