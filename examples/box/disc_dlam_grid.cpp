    ...
    {   
        ...
        for (int i = 0; i < nBoxAmps/2; i++)
        {
            ...
            auto f = [&] (double s, double eta)
            {
                ...
                int b_ind = (i>=6) * 2;
                b = ampB[b_ind]->eval(s, eta) + ampB[b_ind+1]->eval(s,eta);
                ...
                if (c_ind <= 4)
                {
                    c = ampC[c_ind]->eval(s, eta) + ampC[c_ind+1]->eval(s,eta);
                } 
                else
                {
                    // with phase the kth amplitude gets replaced by N-1-kth amplitude where N is TOTAL number of amplitudes
                    c = kpsiD.parity_phase(c_ind, S)*ampC[nPsiD-1 - c_ind]->eval(s,eta) +  kpsiD.parity_phase(c_ind+1, S)*ampC[(nPsiD-1) - (c_ind+1)]->eval(s,eta);
                };

                // finally just multiply by phasespace
                return rho(s) * b * c;
            };
